...
<body>
    ...
    <%
        ...
        try 
		{
            ...
            PreparedStatement pstmt=con.prepareStatement("select count(*) from UserTb where user=? and question=? and answer=?");
            ...
            if(c>0)
            {
                pstmt=con.prepareStatement("Update UserTb set password=? where user=?");
		...
		if(c>0)
        {
			...
			<%
            //response.sendRedirect("LoginPage.html");   will work first as its preference order is much high
        } 
        else
        {
			%>
