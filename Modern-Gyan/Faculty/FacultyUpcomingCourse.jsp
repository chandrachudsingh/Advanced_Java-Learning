...
<title>Faculty Upcoming Courses | Modern-Gyan</title>
    <link rel="stylesheet" href="FacultyUpcomingCourse.css">
...
<body>
    ...
    <%
        ...
        try 
        {
            ...
            PreparedStatement pstmt = con.prepareStatement("SELECT count(*) from "+ faculty_code +"_assignsubject_Tb where assign_date <= ?");
        ...
        if(c == 0)
            out.print("<div id='EmptyMessage'><label> Your upcoming courses which has been approved will appear here. </label></div>");
        else
        {
            ...
            try
            {
                ...
                PreparedStatement pstmt = con.prepareStatement("SELECT platform_id, slotno, subject_code, meeting_id, meeting_password, student_enrolled, assign_date,"
                    + " finish_date from " + faculty_code +"_assignsubject_Tb where assign_date >= ?");
                pstmt.setDate(1, today_date);
    ...
    <%
        ...
        {
            %><script> 
                document.getElementsByClassName('UpcomingCourses').item(0).style.filter='blur(1px)';
                ...
                window.setTimeout('document.getElementById("lbMessage").style.opacity="0"; document.getElementById("lbMessage").style.visibility="hidden";'
                    + ' document.getElementsByClassName("UpcomingCourses").item(0).style.filter="blur(0px)"; document.getElementById("lbMessage").style.boxShadow="none";',4000);
            </script><%
        }
